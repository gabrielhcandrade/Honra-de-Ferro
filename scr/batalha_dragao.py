...
class BatalhaDragao:
    def __init__(self, game: 'Game', tela: pygame.Surface, asset_manager: AssetManager) -> None:
        ...
        self.fundo_batalha = pygame.transform.scale(self.fundo_batalha, (Constantes.LARGURA, Constantes.ALTURA))
    ...
    def _update_fundo_loop(self) -> None:
        ...
        if self.fundo_x1 <= -self.fundo_largura: self.fundo_x1 = self.fundo_x2 + self.fundo_largura
        if self.fundo_x2 <= -self.fundo_largura: self.fundo_x2 = self.fundo_x1 + self.fundo_largura
    ...
    def _gerenciar_fases(self):
        if self._fase_batalha == 1:
            ...
            if agora - self.timer_esqueleto > 3000 and len(self.esqueletos) < 3:
                ...
                esqueleto = Esqueleto(spawn_x, self.jogador.y_chao, self.asset_manager, vida=100)
        ...
        if self._fase_batalha == 2 and self.dragao is None:
            self.dragao = Dragao(x=0, y=0, asset_manager=self.asset_manager, grupo_projeteis=self.fireballs)
                ...
                if abs(self.jogador.rect.centerx - esqueleto.rect.centerx) < 120:
                    ...
                    if not esqueleto.vivo:
                        ...
                        if random.random() < 0.5:
                            pocao = Pocao(esqueleto.rect.center, self.asset_manager)
    ...
    def desenhar(self) -> None:
        # 1. Desenha o fundo e os sprites
        ...
        vida_jogador_surf = self.font_ui.render(vida_jogador_txt, True, Constantes.BRANCO)
        self.tela.blit(vida_jogador_surf, (MARGEM_LATERAL, y_atual))
        y_atual += ESPACO_TEXTO_Y
        
        pocao_txt = self.font_ui.render(f"Poções (H): {self.jogador.num_pocoes}", True, Constantes.BRANCO)
        self.tela.blit(pocao_txt, (MARGEM_LATERAL, y_atual))
        y_atual += ESPACO_TEXTO_Y

        agora = pygame.time.get_ticks()
        cooldown_restante = max(0, (self.jogador.last_dash_time + self.jogador.dash_cooldown) - agora)
        ...
        if self.dragao:
            ...
            pct_dragao = (self.dragao.vida / self.dragao.vida_maxima) * 100 if self.dragao.vida_maxima > 0 else 0
            self.desenhar_barra_vida(self.tela, x_barra_dragao, y_atual_dragao, pct_dragao, (150, 0, 150), Constantes.VERMELHO, LARGURA_BARRA_VIDA, ALTURA_BARRA_VIDA)
